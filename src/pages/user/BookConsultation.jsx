...
export default function BookConsultation() {
  ...
  useEffect(() => {
    ...
    caseAPI.list({ limit: 50 }).then(r => setCases(r.data.data || [])).catch(console.error);
    documentAPI.list({ limit: 100 }).then(r => setDocuments(r.data.data || [])).catch(console.error);
  }, []);

  const selectedAdvisor   = advisors.find(a => a.id === Number(form.advisor_id)) || preAdvisor;
  ...
  const advisorSpecialization = selectedAdvisor?.specializations
    ? (Array.isArray(selectedAdvisor.specializations)
        ? selectedAdvisor.specializations[0]
        : (typeof selectedAdvisor.specializations === 'string'
            ? JSON.parse(selectedAdvisor.specializations)[0]
            : null))
    : null;
        ...
        <div className='row'>
          ...
          <div className='advisor-consultation-cards'>
            ...
            {[
              ...
              { label: 'Fee',        value: selectedAdvisor?.hourly_rate ? `${currency} ${selectedAdvisor.hourly_rate}` : 'Contact advisor' },
            ].map(({ label, value }) => (
            ...
            <button
              ...
              onClick={handleBook}
              disabled={loading || !selectedDate || !selectedTime || !form.advisor_id}
            >
